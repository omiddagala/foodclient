(function () {
    ...
    angular.module('BlurAdmin.pages.ad-rest-cheque-report', [])
        .config(routeConfig)
        .controller('adminRestChequeReport', adminRestChequeReport);

    /** @ngInject */
    function routeConfig($stateProvider) {
        $stateProvider
            .state('ad-rest-cheque-report', {
                url: '/ad-rest-cheque-report',
                templateUrl: 'app/pages/admin/restaurant/cheque-report.html',
                controller: 'adminRestChequeReport'
            });
    }

    function adminRestChequeReport($scope, $filter, editableOptions, editableThemes, $state, $rootScope,$q, $http, localStorageService, $location, $uibModal, $timeout, toastrConfig, toastr) {
        ...
        $scope.search = function (pagination, sort) {
            ...
            return $http.post("https://demoapi.karafeed.com/pepper/v1/financial/getRestaurantChequeList", param, httpOptions)
                .then(function (data, status, headers, config) {
                    stopLoading();
                    $scope.orders = data.data.list;
                    return data.data;
                }).catch(function (err) {
                    $rootScope.handleError(param, "/financial/getRestaurantChequeList", err, httpOptions);
                });
        };
