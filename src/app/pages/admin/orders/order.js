(function () {
    ...
    function adminOrderCtrl($scope, $filter, $state, $q, $http, localStorageService, $uibModal, $rootScope, $uibModalStack, toastrConfig, toastr,$location) {
        ...
        $scope.search = function (pagination, sort) {
            ...
            return $http.post("https://demoapi.karafeed.com/v1/adminEmployeeManagementRest/getOrderById", param, httpOptions)
        ...
        $scope.cancelOrderGroup = function () {
            ...
            $http.post("https://demoapi.karafeed.com/v1/adminEmployeeManagementRest/deleteOrderByGroupId", param, httpOptions)
                .then(function (data, status, headers, config) {
                    stopLoading();
                    $scope.orders = [];
                    showMessage(toastrConfig, toastr, "پیام", "عملیات با موفقیت انجام شد", "success");
