(function () {
    ...
    angular.module('BlurAdmin.pages.res-orders', [])
        .config(routeConfig)
        .controller('resOrdersCtrl', resOrdersCtrl);
    ...
    function routeConfig($stateProvider) {
        $stateProvider
            .state('res-orders', {
                url: '/res-orders',
                templateUrl: 'app/pages/restaurant/orders/res-orders.html',
                controller: 'resOrdersCtrl'
            });
    }

    function resOrdersCtrl($scope, $filter, editableOptions, editableThemes, $state, $q, $http, localStorageService, $timeout, toastrConfig, toastr,$rootScope) {
        ...
        $scope.search = function (pagination, sort) {
            ...
            return $http.post("https://demoapi.karafeed.com/v1/restaurant/food/getRestaurantOrder", param, httpOptions)
                .then(function (data, status, headers, config) {
                    stopLoading();
                    $scope.orders = data.data.list;
                    return data.data;
                }).catch(function (err) {
                    $rootScope.handleError(param, "/restaurant/food/getRestaurantOrder", err, httpOptions);
                });
        };
