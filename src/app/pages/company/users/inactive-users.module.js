(function () {
    ...
    function companyInActiveUsersCtrl($scope, $filter, editableOptions, editableThemes, $state, $rootScope, $q, $http, localStorageService, $location, $uibModal, $uibModalStack, toastrConfig, toastr) {
        ...
        $scope.activateUser = function () {
            ...
            $http.post("https://demoapi.karafeed.com/v1/companyEmployeeManagement/activeEmployee", $scope.companyUserId, httpOptions)
                .then(function (data, status, headers, config) {
                    stopLoading();
        ...
        $scope.doDelete = function () {
            ...
            $http.post("https://demoapi.karafeed.com/v1/companyEmployeeManagement/deleteEmployee", $scope.companyUserId, httpOptions)
                .then(function (data, status, headers, config) {
                    stopLoading();
                    $uibModalStack.dismissAll();
                    $scope.inactiveUsers.splice($scope.companyUserIndex, 1);
        ...
        $scope.doSettle = function () {
            ...
            $http.post("https://demoapi.karafeed.com/v1/companyEmployeeManagement/settlingDeActiveEmployee", $scope.companyUserId, httpOptions)
                .then(function (data, status, headers, config) {
                    stopLoading();
                    $uibModalStack.dismissAll();
                    showMessage(toastrConfig, toastr, "پیام", "عملیات با موفقیت انجام شد", "success");
